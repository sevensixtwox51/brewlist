...
def fetch_deck(deck_id: str) -> dict:
    ...
    try:
        ...
        if e.code == 404:
            raise SystemExit(f"Moxfield deck '{deck_id}' not found (404). Check the URL.")
        raise SystemExit(f"Moxfield API returned HTTP {e.code}: {e.reason}")
    except urllib.error.URLError as e:
        raise SystemExit(f"Could not reach Moxfield API: {e.reason}")
...
def load_collection(path: str) -> dict[str, OwnedCard]:
    ...
    with open(path, newline="", encoding="utf-8-sig") as f:
        ...
        if "Name" not in (reader.fieldnames or []):
            raise SystemExit(
                f"'{path}' doesn't look like a ManaBox export "
                f"(no 'Name' column found). Columns seen: {reader.fieldnames}"
            )
        for row in reader:
...
def load_overrides(deck_id: str, directory: str) -> tuple[dict[str, int], str | None]:
    ...
    report's "Save Overrides" button) in `directory`, e.g. a browser download of
    `{deck_id}_overrides.json`. Picks the most recently modified match if there
    are several (browsers suffix repeat downloads with " (1)", etc). Returns
    (reserved_by_normalized_name, path_used) -- empty dict if none found."""
    directory = os.path.expanduser(directory)
...
def fetch_scryfall_prices_by_id(scryfall_ids: set[str]) -> dict[str, dict]:
    ...
    with console.status(f"Pricing {len(ids)} owned card(s) via Scryfall...") as status:
        for i, scryfall_id in enumerate(ids, 1):
            status.update(f"Pricing owned cards via Scryfall... ({i}/{len(ids)})")
            req = urllib.request.Request(
...
def shopping_group_rank(group: str, subgroup: str) -> str:
    ...
    correctly: Lands, Artifacts, then Colors (mono x5, Multicolor, Colorless)."""
...
def build_comparison(
    ...
    price_cache = fetch_scryfall_prices_by_id(needed_ids)
...
<header>
  ...
  <div class="controls">
    ...
    {shopping_button}
    {instore_button}
    <button class="btn ghost" id="save-overrides-btn" title="Download a JSON file remembering which owned cards are reserved for other decks -- drop it in your ManaBox export folder and future runs will account for it automatically">&#128190; Save Overrides</button>
  </div>
</header>
<main>
{buckets_html}
...
def render_html(deck_name: str, deck_url: str, deck_id: str, bucket_names: list[str],
                 buckets: dict[str, list[CardResult]], totals: dict) -> str:
