...
def run_ai_build(
    ...
    system_prompt = (
        ...
        "WORKFLOW -- this matters: searching does not build the deck, add_card does. After every search (or "
        "every 1-2 searches at most), call add_card for the best matches you just found before searching again. "
        "Do not run more than two searches in a row without adding anything -- if a search returns real "
        "candidates, add some of them immediately rather than continuing to look for something better. Prefer "
        "broad searches (a mechanic, a creature type, a card type like \"land\" or \"removal\") over guessing "
        "individual famous card names one at a time -- a broad search already surfaces real, owned options to "
        "act on. Your only goal is reaching exactly {target} library cards; time spent only searching without "
        "adding makes zero progress toward that, no matter how thorough it feels. Call check_combos every "
